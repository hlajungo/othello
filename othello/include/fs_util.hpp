...
public:
  /*
   * @brief Go to `dir` find file that with `file_prefix`
   */
  array_1d_t<std::string> get_file_1d(const std::string& dir, const std::string& file_prefix) {
    array_1d_t<std::string> file_1d;
    for(const auto& entry: std_fs::directory_iterator(dir)) {
      if(entry.is_regular_file()) {
        std::string filename= entry.path().filename().string();
        // 檢查前綴
        if(filename.rfind(file_prefix, 0) == 0) { file_1d.push_back(entry.path().string()); }
      }
    }
    return file_1d;
  }
  /*
   * @brief `target` in file `filename`
   * @return true: founded, false: not founded.
   */
  template<typename T> bool is_in_file(const std::string& filename, const T& target) {
    static_assert(std::is_trivially_copyable_v<T>, "is_in_file only supports trivially copyable types");
  ...
  bool is_file_equal(const std::string& path1, const std::string& path2) {
    ...
    do {
      ...
      std::streamsize bytes1= f1.gcount();
      std::streamsize bytes2= f2.gcount();
      if(bytes1 != bytes2) return false; // 檔案大小不同
      if(!std::equal(buf1, buf1 + bytes1, buf2)) return false;
    } while(f1 && f2);
    return true;
  }
  void try_mkdir(std_fs::path& path) {
    if(!std_fs::exists(path)) std_fs::create_directories(path);
  }
};
inline Fs_util fs_util;
// Linux mmap optimize for reading file
#include <fcntl.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <unistd.h>
template<typename T> bool is_in_file2(std::string_view filename, const T& target) {
  static_assert(std::is_trivially_copyable_v<T>, "is_in_file only supports trivially copyable types");
