...
public:
   ...
   */
  void
  hash_position (uint64_t& hash, const Game_ctx& game_ctx)
  {
    // check for nullptr
    //assert (game_ctx_ptr);
    //auto& game_ctx = *game_ctx_ptr;
...
// 能在模板注入時，確保該類型存在 API。
template <typename T>
concept Hashable = requires (T target, uint64_t h, Game_ctx game_ctx)
{
  {
    target.hash_position (h, game_ctx)
  } -> std::same_as<void>;
};

template <Hashable Hash_impl>
class Hash
{
  Hash_impl& hash;
};
